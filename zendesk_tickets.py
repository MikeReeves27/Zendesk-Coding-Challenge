...
def download_tickets(credentials):
    '''Downloads all tickets from json file and stores them in a list of dictionaries. Return list'''
...
# Menu selection input function:
def menu_selection():
    '''Returns user's input for menu selection'''
    user_selection = input('Selection: ')
    print()
    return user_selection


# Display ticket function
def display_all_tickets(ticket_list):
    ''' Displays all tickets from the json file'''
    ...
    while ticket_index < len(ticket_list):
        ...
        for ticket in range(ticket_index, ticket_index + 25):
            ...
            print(str(ticket_list[ticket]['id']) + '\t\t' + str(ticket_list[ticket]['priority']) + '\t\t' + ticket_list[ticket]['status'] + \
                  '\t\t' + ticket_list[ticket]['created_at'] + '\t\t' + str(ticket_list[ticket]['requester_id']) + '\t\t' + ticket_list[ticket]['subject'])
...
# Search for ticket function:
def search_for_ticket(ticket_list):
    '''Searches for an individual ticket number from user input'''
    ...
    for ticket in ticket_list:
        ...
        # If the ticket number exists in the list and is paired with a ticket id (that is, the value the user inputs is not just 
        ...
        if ('id', ticket_number) in ticket.items():
            ...
            print('TICKET #\tPRIORITY\tSTATUS\t\tTIME CREATED\t\t\tREQUESTER ID\t\tSUBJECT')
            print('--------\t--------\t------\t\t------------\t\t\t------------\t\t-------')
            print(str(ticket['id']) + '\t\t' + str(ticket['priority']) + '\t\t' + ticket['status'] + '\t\t' + ticket['created_at'] + \
                  '\t\t' + str(ticket['requester_id']) + '\t\t' + ticket['subject'])
            ticket_exists = True
            break
...
def main():
    ...
    # Run login() function to grab user credentials
    credentials = login()

    # Pass user credentials into download_tickets() function
    ticket_list = download_tickets(credentials)

    # Run loop so that user can keep making menu selections until they exit program
    ...
    while True:
        ...
        # Run menu_selection() function to get user input
        user_selection = menu_selection()

        # If user enters 1, display all tickets
        if user_selection == '1':
            display_all_tickets(ticket_list)

        # If user enters 2, search for a ticket
        elif user_selection == '2':
            search_for_ticket(ticket_list)
